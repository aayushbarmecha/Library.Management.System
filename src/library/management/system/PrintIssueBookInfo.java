...
public class PrintIssueBookInfo {
    public void export(){
        try {
            conn con = new conn();
            ...
            XSSFSheet sheet = wb.createSheet("Issue Book Information");

            writeHeaderLine(sheet);
            writeDataLines(rs, wb, sheet);

            String outputPath = System.getProperty("user.home")+"\\Documents";
            FileOutputStream output = new FileOutputStream(outputPath+"\\IssueBookInformation.xlsx");
            wb.write(output);
            output.close();

            JOptionPane.showMessageDialog(null,"Issue Book Information has been exported to "+outputPath+"\nFilename: IssueBookInformation.xlsx", "Export Success",JOptionPane.PLAIN_MESSAGE);
    ...
    private void writeHeaderLine(XSSFSheet sheet){
        ...
        headerCell.setCellValue("Date of Expected Return");
    }
    ...
    private void writeDataLines(ResultSet result, XSSFWorkbook workbook, XSSFSheet sheet) throws SQLException{
        ...
        while(result.next()){
            ...
            String dateReturn = result.getString("dateOfExpectedReturn");
